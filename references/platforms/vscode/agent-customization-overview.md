<!--
source: https://code.visualstudio.com/docs/agent-customization/overview
fetched: 2026-08-11
status: complete
notes: No redirect (URL resolved to itself). Stripped site chrome (top nav, left Documentation/Topics tree, "Copy as Markdown" widget, "On this page" list, "Try this" promo cards, footer social/legal links, trailing "8/5/2026" date stamp). Inline setting links rendered in the source as "<setting-id> Open in VS Code Open in VS Code Insiders" and were normalized to backticked setting IDs. No org-policy ("ORG") badges were present on any setting on this page. Source code fences carried no language tags; preserved as bare fences.
-->

# Create and manage agent customizations

Agent customizations adapt agents to your coding standards, workflows, and development systems. This article explains how to create, manage, evaluate, and troubleshoot customizations in Visual Studio Code. To compare the customization types and understand how they work together, see [Agent customization concepts](https://code.visualstudio.com/docs/agents/concepts/customization).
...
When you open the Customizations editor, the customizations are scoped to the selected [agent harness](https://code.visualstudio.com/docs/agents/concepts/agent-harnesses). Select the harness from the dropdown in the chat input before you open the editor to ensure the customizations apply to the correct context.

![Screenshot showing the Agent Customizations panel in the Agents window, with the list of available customizations visible.](https://code.visualstudio.com/assets/docs/agent-customization/customization/agents-customizations.png)
...
![Screenshot showing the Agent Customizations panel in the Agents window, with the list of available customizations visible.](https://code.visualstudio.com/assets/docs/agent-customization/customization/agents-window-customizations.png)
...
2. Choose between creating a user or workspace customization from the **New** dropdown

3. Enter a name and choose a storage location when prompted

4. An inline editor opens where you can further edit the customization file. The editor provides syntax highlighting and validation for the customization type.
...
When [Agent Host](https://code.visualstudio.com/docs/agents/concepts/agent-host) is enabled, the agent reads user-level customizations from harness-agnostic folders like `~/.copilot` (Copilot) and `~/.claude` (Claude), rather than from your VS Code profile user data. See [instructions](https://code.visualstudio.com/docs/agent-customization/custom-instructions#_instructions-file-locations), [custom agents](https://code.visualstudio.com/docs/agent-customization/custom-agents), and [prompt files](https://code.visualstudio.com/docs/agent-customization/prompt-files#_prompt-file-locations) for the recommended user-level locations.
...
- The parent repository folder is [trusted](https://code.visualstudio.com/docs/editing/workspaces/workspace-trust). VS Code prompts you to trust the parent folder when the workspace is opened.
