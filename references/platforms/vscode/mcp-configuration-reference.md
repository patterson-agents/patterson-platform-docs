...
fetched: 2026-08-11
status: complete
notes: Site navigation chrome (top nav, left "Documentation"/"Topics" tree, "Copy as Markdown" widget, "On this page" list, footer social/legal links) stripped; article body only. Inline setting links rendered as garbled text on fetch (e.g. "chat.mcp.access  Open in VS Code Open in VS Code InsidersThis setting can be managed by your organization. Contact your administrator to change it.ORG") and were normalized to a backticked setting ID plus [ORG] where the org-managed badge was present. Content cross-checked against the upstream docs source (microsoft/vscode-docs, docs/agents/reference/mcp-configuration.md, DateApproved 7/15/2026); the live page is newer (8/5/2026) and adds the Agent Host note under "Configuration file". Page footer date: 8/5/2026.
-->

# MCP configuration reference

This article provides a reference for the MCP server configuration file format, related commands, and settings in VS Code. For information about adding and managing MCP servers, see [Add and manage MCP servers](https://code.visualstudio.com/docs/agent-customization/mcp-servers).

## Configuration file

MCP server configuration is stored in the `mcp.json` JSON file. This file can be in your workspace (`.vscode/mcp.json`) or in your [user profile](https://code.visualstudio.com/docs/configure/profiles). VS Code provides IntelliSense for the configuration file.

> **Note**
>
> VS Code forwards the servers you configure to the [Agent Host](https://code.visualstudio.com/docs/agents/concepts/agent-host), except servers that require interactive input (for example, `${input:...}` variables). The Agent Host doesn't read `.vscode/mcp.json` directly; for portable configuration, use a workspace `.mcp.json` or user `~/.copilot/mcp-config.json` file, which the Agent Host reads natively. See [behavior on the extension host](https://code.visualstudio.com/docs/agents/concepts/agent-host#_behavior-on-the-extension-host).
...
You can use [predefined variables](https://code.visualstudio.com/docs/reference/variables-reference), such as `${workspaceFolder}`, in file system path values.
...
Learn more about [MCP development mode](https://code.visualstudio.com/api/extension-guides/ai/mcp#mcp-development-mode-in-vs-code) in the MCP Dev Guide.
...
For a full list of VS Code AI settings, see the [AI Settings Reference](https://code.visualstudio.com/docs/agents/reference/ai-settings). The following settings are specific to MCP servers.

| Setting | Description |
|---------|-------------|
| `chat.mcp.access` [ORG] | Manage which MCP servers can be used in VS Code. |
| `chat.mcp.discovery.enabled` | Configure automatic discovery of MCP server configuration from other applications. |
| `chat.mcp.autostart` (Experimental) | Automatically start MCP servers when configuration changes are detected. |
| `chat.mcp.serverSampling` | Configure which models are exposed to MCP servers for sampling (making requests in the background). |
| `chat.mcp.apps.enabled` (Experimental) | Enable or disable MCP Apps, which are rich user interfaces provided by MCP servers. |

> `[ORG]` marks settings that carried the badge "This setting can be managed by your organization. Contact your administrator to change it." on the live page.

## Related resources

- [Add and manage MCP servers](https://code.visualstudio.com/docs/agent-customization/mcp-servers)
- [Model Context Protocol Documentation](https://modelcontextprotocol.io/)
- [MCP Dev Guide](https://code.visualstudio.com/docs/agents/guides/mcp-developer-guide)

_Page last updated: 8/5/2026_
