<!--
source: https://code.visualstudio.com/docs/enterprise/extensions
fetched: 2026-08-11
status: complete
notes: No redirect (final URL identical to requested URL). Stripped site chrome: top nav, left "Documentation"/"Topics" tree, "Copy as Markdown" widget, "On this page" list, footer social/legal links, banner. Normalization applied: inline setting links rendered in the source as "extensions.allowed  Open in VS Code Open in VS Code InsidersThis setting can be managed by your organization. Contact your administrator to change it.ORG" were normalized to `extensions.allowed` [ORG] (two occurrences, both carrying the org-managed badge). Note/Tip/Important callouts rendered as blockquotes. Page metadata ms.date 8/5/2026. No tables present in the source article. No mention of `ExtensionGalleryServiceUrl` appears on this page.
-->
...
Organizations can centrally manage allowed extensions by using the `AllowedExtensions` [policy](https://code.visualstudio.com/docs/enterprise/policies). Through device management solutions, admins can deploy and enforce the policy across all managed devices. This overrides any user-configured `extensions.allowed` [ORG] setting on individual devices.

![Screenshot of configuring AllowedExtensions from the Local Group Policy Editor.](https://code.visualstudio.com/assets/docs/enterprise/policies/allowed-extensions-local-gp-editor.png)
...
2. Download the [VSIX files](https://code.visualstudio.com/docs/configure/extensions/extension-marketplace#_can-i-download-an-extension-directly-from-the-marketplace) for the extensions that you want to preinstall and place them in the `bootstrap\extensions` folder.
...
- **Upstreaming**: Automatically include public extensions from the [Visual Studio Marketplace](https://marketplace.visualstudio.com), even in environments with restricted internet connectivity. Allow or deny select extensions by setting up an [allow list](#_configure-allowed-extensions).
...
- [Extension Marketplace](https://code.visualstudio.com/docs/configure/extensions/extension-marketplace) - Learn about installing and managing extensions in VS Code.
- [Extension runtime security](https://code.visualstudio.com/docs/configure/extensions/extension-runtime-security) - Learn about extension security in VS Code.
- [Enterprise policies](https://code.visualstudio.com/docs/enterprise/policies) - Reference for all enterprise policies in VS Code.
