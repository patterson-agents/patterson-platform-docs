...
fetched: 2026-08-11
status: complete
notes: No redirect (URL resolved to itself); the legacy URL https://code.visualstudio.com/docs/copilot/chat/chat-modes redirects HERE. Page confirms "Custom agents were previously known as custom chat modes" and that `.chatmode.md` files should be renamed to `.agent.md`. Stripped site chrome (top nav, left Documentation/Topics tree, "Copy as Markdown" widget, "On this page" list, footer social/legal links, trailing "8/5/2026" date stamp). Inline setting links rendered in the source as "<setting-id> Open in VS Code Open in VS Code Insiders" and were normalized to backticked setting IDs. No org-policy ("ORG") badges were present on any setting on this page. Source code fences carried no language tags; preserved as bare fences.
-->
...
The [built-in agent roles](https://code.visualstudio.com/docs/agents/run/agent-harnesses#_choose-a-built-in-agent-role) provide general-purpose configurations for chat in VS Code. For a more tailored chat experience, you can create your own custom agents.
...
Subagents can run with a custom agent. Learn more about running [subagents with custom agents](https://code.visualstudio.com/docs/agents/run/subagents#_run-a-custom-agent-as-a-subagent).
...
When [Agent Host](https://code.visualstudio.com/docs/agents/concepts/agent-host) is enabled, the agent reads user-level custom agents from `~/.copilot/agents` and not from VS Code profile user data. To use existing user-level custom agents with the Copilot agent, store them in `~/.copilot/agents`.
...
| `tools`                    | A list of tool or [tool set](https://code.visualstudio.com/docs/agent-customization/tool-sets) names that are available for this custom agent. Can include built-in tools, tool sets, MCP tools, or tools contributed by extensions. To include all tools of an MCP server, use the `<server name>/*` format.   Learn more about [tools with agents](https://code.visualstudio.com/docs/agents/run/tools). |
| `agents`                   | A list of agent names that are available as [subagents](https://code.visualstudio.com/docs/agents/run/subagents) in this agent. Use `*` to allow all agents, or an empty array `[]` to prevent any subagent use. If you specify `agents`, ensure the `agent` tool is included in the `tools` property. To create a self-referential agent that lists itself in `agents`, enable `chat.subagents.allowInvocationsFromSubagents`. Learn more about [nested subagents](https://code.visualstudio.com/docs/agents/run/subagents#_nested-subagents). |
...
| `user-invocable`           | Optional boolean flag to control whether the agent appears in the agents dropdown in chat (default is `true`). Set to `false` to create agents that are only accessible as [subagents](https://code.visualstudio.com/docs/agents/run/subagents) or programmatically. |
...
| `hooks` (Preview)          | Optional hook commands scoped to this agent. Hooks defined here only run when this agent is active, either invoked by the user or as a subagent. Uses the same format as [hook configuration files](https://code.visualstudio.com/docs/agent-customization/hooks#_hook-configuration-format). Requires `chat.useCustomAgentHooks` to be enabled. |
...
2. Select **New Agent (Workspace)** or **New Agent (User)** from the dropdown, depending on where you want to store the agent file.
    ...
    ![Screenshot of the Agent Customizations editor, showing the Agents tab and the dropdown to create a new custom agent.](https://code.visualstudio.com/assets/docs/agent-customization/customization/create-custom-agent.png)

    Alternatively, run the **Chat: New Custom Agent** command from the Command Palette (⇧⌘P (Windows, Linux Ctrl+Shift+P)).

  Tip     You can configure additional locations where VS Code searches for custom agent files by using the `chat.agentFilesLocations` setting. This is useful for sharing agents across projects or keeping them in a central location outside your workspace.
